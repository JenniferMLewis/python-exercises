...
capititalized_fruits = [fruit.title() for fruit in fruits]
print(capititalized_fruits)
...
for fruit in fruits:
    for l in fruit:
        if l == 'a':
            if fruit not in fruits_with_letter_a:
...
two_or_more_digits = [number for number in numbers if len(str(number).replace("-","")) >= 2]
